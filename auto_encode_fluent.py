...
class SettingsDialog(QDialog):
    ...
    def _load_general_data(self):
        ...
        self.chk_ep_not_shared.setChecked(self.raw_settings.get("ep_not_shared", True))
...
class EncodeManager(QMainWindow):
    ...
    def _on_subtitle_mode_changed(self, route_key: str):
        ...
        old_suffix = self._get_param_key_suffix(old_has_subtitle)
        profile[f"selected_hw_{old_suffix}"] = self.seg_hardware.currentItem().text()
        profile[f"selected_codec_{old_suffix}"] = self.seg_codec.currentItem().text()
        profile[f"suffix_enabled_{old_suffix}"] = self.chk_suffix.isChecked()
        profile[f"suffix_{old_suffix}"] = self.entry_suffix.text()
        ...
        new_suffix = self._get_param_key_suffix(new_has_subtitle)
        sel_hw = profile.get(f"selected_hw_{new_suffix}", profile.get("selected_hw", "CPU"))
        sel_codec = profile.get(f"selected_codec_{new_suffix}", profile.get("selected_codec", "X264"))
        self.seg_hardware.blockSignals(True)
        self.seg_codec.blockSignals(True)
        self.seg_hardware.setCurrentItem(sel_hw)
        self.seg_codec.setCurrentItem(sel_codec)
        self.seg_hardware.blockSignals(False)
        self.seg_codec.blockSignals(False)
        self._refresh_text_fields(sel_hw, sel_codec)
        
        new_suffix_enabled = profile.get(f"suffix_enabled_{new_suffix}", False)
        new_suffix_text = profile.get(f"suffix_{new_suffix}", "[V2]")
        self.chk_suffix.blockSignals(True)
        self.entry_suffix.blockSignals(True)
        self.chk_suffix.setChecked(new_suffix_enabled)
        self.entry_suffix.setText(new_suffix_text)
        self.entry_suffix.setEnabled(new_suffix_enabled)
        self.chk_suffix.blockSignals(False)
        self.entry_suffix.blockSignals(False)
    ...
    # ==================== 参数切换与保存核心逻辑 ====================
    def _get_param_key_suffix(self, has_subtitle: bool) -> str:
        return "with_sub" if has_subtitle else "no_sub"
    ...
    def _on_selector_changed(self, _=None):
        ...
        has_subtitle = self.profiles[self.current_profile_name].get("has_subtitle_mode", True)
        suffix = self._get_param_key_suffix(has_subtitle)
        self.profiles[self.current_profile_name][f"selected_hw_{suffix}"] = hw
        self.profiles[self.current_profile_name][f"selected_codec_{suffix}"] = codec
        self._save_profiles_to_disk()
    ...
    def _refresh_text_fields(self, hw: str, codec: str):
        ...
        suffix = self._get_param_key_suffix(has_subtitle)
        params_matrix = profile.get(f"param_matrix_{suffix}", profile.get("param_matrix", DEFAULT_PARAMS_TEMPLATE))
    ...
    def _save_current_params(self):
        ...
        has_subtitle = profile.get("has_subtitle_mode", True)
        suffix = self._get_param_key_suffix(has_subtitle)
        key = f"param_matrix_{suffix}"
        
        if key not in profile:
            profile[key] = copy.deepcopy(DEFAULT_PARAMS_TEMPLATE)
            
        if hw not in profile[key]: profile[key][hw] = {}
        if codec not in profile[key][hw]: profile[key][hw][codec] = {}
        
        target = profile[key][hw][codec]
    ...
    def _save_current_profile_meta(self):
        ...
        profile.update({
            ...
            "mux_suffix_enabled": self.mux_chk_suffix.isChecked(),
            "mux_suffix": self.mux_entry_suffix.text()
        })
        self._save_profiles_to_disk()
    ...
    def _add_profile_dialog(self):
        ...
        self.profiles[name] = {
            ...
            "out_sc": "", "out_tc": "",
            "param_matrix_with_sub": profile_matrix,
            "param_matrix_no_sub": copy.deepcopy(defaults),
            "selected_hw_with_sub": "CPU",
            "selected_codec_with_sub": "X264",
            "selected_hw_no_sub": "CPU",
            "selected_codec_no_sub": "X264",
            "suffix_enabled_with_sub": False,
            "suffix_with_sub": "[V2]",
            "suffix_enabled_no_sub": False,
            "suffix_no_sub": "[V2]",
            "last_ep": "01",
            "last_ep_no_sub": "01",
            ...
            "mux_sub_tc_default": False, "mux_suffix_enabled": False, "mux_suffix": "[V2]"
        }
    ...
    def _select_profile(self, name: str):
        ...
        self.mux_chk_suffix.setChecked(data.get("mux_suffix_enabled", False))
        self.mux_entry_suffix.setText(data.get("mux_suffix", "[V2]"))
        self.mux_entry_suffix.setEnabled(data.get("mux_suffix_enabled", False))
        
        suffix = self._get_param_key_suffix(has_subtitle)
        sel_hw = data.get(f"selected_hw_{suffix}", data.get("selected_hw", "CPU"))
        sel_codec = data.get(f"selected_codec_{suffix}", data.get("selected_codec", "X264"))
        self.seg_hardware.setCurrentItem(sel_hw)
        self.seg_codec.setCurrentItem(sel_codec)
        
        suffix_enabled = data.get(f"suffix_enabled_{suffix}", False)
        suffix_text = data.get(f"suffix_{suffix}", "[V2]")
        self.chk_suffix.setChecked(suffix_enabled)
        self.entry_suffix.setText(suffix_text)
        self.entry_suffix.setEnabled(suffix_enabled)
    ...
    def _block_signals(self, block: bool):
        widgets = [
            ...
            self.chk_2pass, self.chk_suffix, self.entry_suffix,
            ...
            self.mux_sub_tc_default, self.mux_chk_suffix, self.mux_entry_suffix
        ]
    ...
    def _load_profiles(self):
        ...
        try:
            ...
            for pname, pdata in self.profiles.items():
                if "param_matrix" not in pdata and "param_matrix_with_sub" not in pdata:
                    ...
                    pdata["param_matrix_with_sub"] = new_matrix
                    pdata["param_matrix_no_sub"] = copy.deepcopy(new_matrix)
                    old_hw = pdata.get("selected_hw", "CPU")
                    old_codec = pdata.get("selected_codec", "X264")
                    pdata["selected_hw_with_sub"] = old_hw
                    pdata["selected_codec_with_sub"] = old_codec
                    pdata["selected_hw_no_sub"] = old_hw
                    pdata["selected_codec_no_sub"] = old_codec
                elif "param_matrix" in pdata and "param_matrix_with_sub" not in pdata:
                    ...
                    pdata["param_matrix_with_sub"] = copy.deepcopy(pdata["param_matrix"])
                    pdata["param_matrix_no_sub"] = pdata.pop("param_matrix")
                    old_hw = pdata.pop("selected_hw", "CPU")
                    old_codec = pdata.pop("selected_codec", "X264")
                    pdata["selected_hw_with_sub"] = old_hw
                    pdata["selected_codec_with_sub"] = old_codec
                    pdata["selected_hw_no_sub"] = old_hw
                    pdata["selected_codec_no_sub"] = old_codec
                    ...
                    for suffix in ["with_sub", "no_sub"]:
                        mk = f"param_matrix_{suffix}"
                        if mk in pdata:
                            for hw in pdata[mk]:
                                for codec in pdata[mk][hw]:
                                    if "enable_2pass" not in pdata[mk][hw][codec]:
                                        pdata[mk][hw][codec]["enable_2pass"] = False
                ...
                if "out_no_sub" not in pdata:
                    pdata["out_no_sub"] = ""
                if "suffix_enabled_with_sub" not in pdata:
                    pdata["suffix_enabled_with_sub"] = False
                    pdata["suffix_with_sub"] = "[V2]"
                if "suffix_enabled_no_sub" not in pdata:
                    pdata["suffix_enabled_no_sub"] = False
                    pdata["suffix_no_sub"] = "[V2]"
    ...
    def _add_task_internal(self, ep: str, mode: str, suffix_text: str):
        ...
        for task in tasks:
            ...
            if task['type'] == "NO_SUB":
                hw = profile.get("selected_hw_no_sub", profile.get("selected_hw", "CPU"))
                codec = profile.get("selected_codec_no_sub", profile.get("selected_codec", "X264"))
                p_matrix = profile.get("param_matrix_no_sub", profile.get("param_matrix", DEFAULT_PARAMS_TEMPLATE))
            else:
                hw = profile.get("selected_hw_with_sub", profile.get("selected_hw", "CPU"))
                codec = profile.get("selected_codec_with_sub", profile.get("selected_codec", "X264"))
                p_matrix = profile.get("param_matrix_with_sub", profile.get("param_matrix", DEFAULT_PARAMS_TEMPLATE))
    ...
    def _run_ffmpeg(self, task: Dict):
        ...
        if task_type == "NO_SUB":
            hw = profile.get("selected_hw_no_sub", profile.get("selected_hw", "CPU"))
            codec = profile.get("selected_codec_no_sub", profile.get("selected_codec", "X264"))
            params_matrix = profile.get("param_matrix_no_sub", profile.get("param_matrix", DEFAULT_PARAMS_TEMPLATE))
        else:
            hw = profile.get("selected_hw_with_sub", profile.get("selected_hw", "CPU"))
            codec = profile.get("selected_codec_with_sub", profile.get("selected_codec", "X264"))
            params_matrix = profile.get("param_matrix_with_sub", profile.get("param_matrix", DEFAULT_PARAMS_TEMPLATE))
    ...
    def _create_mux_section(self) -> CardWidget:
        ...
        self.mux_chk_suffix = CheckBox("添加后缀")
        bottom_layout.addWidget(self.mux_chk_suffix)
        self.mux_entry_suffix = PlainLineEdit()
        self.mux_entry_suffix.setPlaceholderText("[V2]")
        self.mux_entry_suffix.setText("[V2]")
        self.mux_entry_suffix.setFixedWidth(80)
        self.mux_entry_suffix.setEnabled(False)
        self.mux_chk_suffix.stateChanged.connect(
            lambda s: self.mux_entry_suffix.setEnabled(s == 2)
        )
        bottom_layout.addWidget(self.mux_entry_suffix)
    ...
    def _add_mux_task(self):
        ...
        profile = copy.deepcopy(self.profiles[self.current_profile_name])
        use_suffix = self.mux_chk_suffix.isChecked()
        suffix_text = self.mux_entry_suffix.text().strip() if use_suffix else ""
        display_suffix = f" {suffix_text}" if suffix_text else ""
        task_data = {
            "type": "mux",
            "ep": ep,
            "profile": profile,
            "suffix": suffix_text,
            "desc": f"[混流] EP{ep}{display_suffix} - {self.current_profile_name}"
        }
    ...
    def _execute_mux_task(self, task_data: Dict, list_item: QListWidgetItem):
        ...
        if task_type == "extract":
            ...
            self._do_mux_task(ep, profile, list_item, suffix)
    ...
    def _do_mux_task(self, ep: str, profile: Dict, list_item: QListWidgetItem, suffix: str = ""):
        mkvmerge_path = self.app_settings.get("mkvmerge_path", "")
        if not mkvmerge_path or not os.path.exists(mkvmerge_path):
            InfoBar.error(title="错误", content="请先在设置中配置 mkvmerge.exe 路径", parent=self)
            return
        ...
        if subset_dir:
            ...
            if os.path.exists(actual_subset_dir):
                ...
                for font_file in font_files:
                    ...
                    cmd_parts.extend(['--attachment-name', f'"{font_name}"', '--attachment-mime-type', mime_type, '--attach-file', f'"{font_file}"'])
        ...
        self._mux_worker.log_signal.connect(self.mux_log_text.append)
        self._mux_worker.finished_signal.connect(lambda s, m: self._on_mux_finished(s, list_item, suffix))
        self._mux_worker.start()

    def _on_mux_finished(self, success: bool, list_item: QListWidgetItem, suffix: str = ""):
        try:
            if list_item is not None:
                widget = self.mux_queue_list.itemWidget(list_item)
                if widget is not None and isinstance(widget, TaskItemWidget):
        ...
        if success and not suffix and self.app_settings.get("auto_ep_mux", True):
            ep = self.mux_ep.text().strip()
            if ep:
                try:
                    next_ep = str(int(ep) + 1).zfill(len(ep))
    ...
    def _start_mux(self):
        ...
        use_suffix = self.mux_chk_suffix.isChecked()
        suffix_text = self.mux_entry_suffix.text().strip() if use_suffix else ""
        self._do_mux_task(ep, profile, None, suffix_text)
